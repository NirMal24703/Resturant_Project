 ...
 * useScrollReveal
 * ---------------
 * Fades sections up as they enter the viewport. Elements are opted in either
 * by carrying a `data-reveal` attribute already, or by matching `selector`.
 *
 * Two safety rails, because an invisible page is worse than an unanimated one:
 *   • the attribute is written in a layout effect, so nothing flashes first;
 *   • if the observer never fires at all, a timer reveals everything.
 */
